...
export const validateActor = (actor) => {
  if (!actor.class) {
    ui.notifications.error(game.i18n.localize("PF2E_LEVEL_UP_WIZARD.notifications.missingClass"));
    return false;
  }
  ...
  if (actor.class.flags.babele?.translated && actor.class.name.indexOf("-") > -1) {
    ui.notifications.error(game.i18n.localize("PF2E_LEVEL_UP_WIZARD.notifications.translatedDualClassError"));
    return false;
  }
  return true;
};
...
export const confirmChanges = async () => {
  return foundry.applications.api.DialogV2.confirm({
    ...
    content: `<p>${game.i18n.localize("PF2E_LEVEL_UP_WIZARD.menu.confirmDialog.content")}</p>`,
    modal: true
  });
};

export const createGlobalLevelMessage = (actorName, targetLevel, selectedFeats, skillIncreaseMessage) => {
  const globalMessage = `
  <h2>${game.i18n.format("PF2E_LEVEL_UP_WIZARD.messages.global.header", {
    actorName,
    targetLevel
  })}</h2>
  <p><strong>${game.i18n.localize("PF2E_LEVEL_UP_WIZARD.messages.global.feats")}</strong> ${
    selectedFeats || `${game.i18n.localize("PF2E_LEVEL_UP_WIZARD.messages.global.noFeats")}`
  }</p>
  ${
  ...
  if (formData.spellcasting) {
    manualUpdates.push(
      formData.newSpellRankLevel
        ...
        : game.i18n.localize("PF2E_LEVEL_UP_WIZARD.messages.personal.spellSlots")
    );
  }

  if (formData.attributeBoosts?.includes("int")) {
    manualUpdates.push(game.i18n.localize("PF2E_LEVEL_UP_WIZARD.messages.personal.intIncrease"));
  }
...
export const getClassJournal = async (actor) => {
  ...
  const classNames = characterClass.includes("-")
    ? characterClass.split("-").map((className) => className.trim().toLowerCase())
    : [characterClass];

  const classesJournal = game.packs.get("pf2e.journals")?.index.find((entry) => entry.name === "Classes");
  ...
  const classSpecificJournals = classNames.map((className) =>
    classesJournalEntry.pages.contents.find((page) => page.name.toLowerCase() === className)
  );
...
export const createFeatChatMessage = (feat) => {
  ...
  const actionGlyph = getActionGlyph(feat.system.actionType?.value, feat.system.actions?.value);
  ...
  const traitsTags = traits
    .map(
      (trait) =>
        `<span class="tag" data-trait data-tooltip="PF2E.TraitDescription${capitalize(trait)}">${capitalize(
          trait
        )}</span>`
    )
    .join("");
  ...
  const chatContent = `
    <div class="pf2e chat-card item-card" data-actor-id="${actorId}" data-item-id="${itemId}">
        ...
        <footer>
            <span>${game.i18n.localize("PF2E_LEVEL_UP_WIZARD.messages.global.feat")} ${feat.system.level.value}</span>
        </footer>
    </div>
  `;
